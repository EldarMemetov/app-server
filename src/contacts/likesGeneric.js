...
export const toggleLike = async ({
  ...
  try {
    ...
    const targetDoc = await map.model.findById(targetId).session(session);
    if (!targetDoc) throw createHttpError(404, `${targetType} not found`);
    ...
    const likesCount = await LikeCollection.countDocuments({
      targetType,
      targetId,
    }).session(session);
    ...
    const ownerId = map.getOwnerId(targetDoc);
    ...
    if (ownerId && String(ownerId) !== String(fromUserId) && liked) {
      const fromUser = await UserCollection.findById(fromUserId).select(
        'name surname photo',
      );
      ...
      if (targetType === 'user') {
        title = `${fromUser.name} liked your profile`;
        message = `${fromUser.name} ${fromUser.surname}`;
        meta.profileUrl = `/talents/${fromUserId}`;
      }

      if (targetType === 'post') {
        title = `${fromUser.name} liked your post`;
        message = `${fromUser.name} ${fromUser.surname}`;
        meta.postUrl = `/posts/${targetId}`;
      }

      if (targetType === 'comment') {
        title = `${fromUser.name} liked your comment`;
        message = `${fromUser.name} ${fromUser.surname}`;
        meta.commentId = targetId;
      }
      ...
      await createNotification({
        ...
        fromUser: fromUserId,
        type: 'like',
        key: `like_${targetType}_${fromUserId}_${targetId}`,
        title,
        message,
        meta,
        unique: true,
        uniqueMetaKeys: ['commentId', 'postUrl', 'profileUrl'],
      });
    }
