...
export const findSessionByAccessToken = (accessToken) =>
  SessionCollection.findOne({ accessToken, revoked: { $ne: true } });
...
export const getSessionsForUser = (userId) =>
  SessionCollection.find({ userId }).select('-refreshToken').lean();

export const revokeSessionById = async (sessionId, userId) => {
  const session = await SessionCollection.findOne({ _id: sessionId, userId });
